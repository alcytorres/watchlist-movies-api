# Reusable import: add movie titles from tmp/favorite_titles.txt to Favorites for user id 1.
# Usage:
#   1. Put one movie title per line in tmp/favorite_titles.txt
#   2. bundle exec rails runner script/import_favorites.rb
require "net/http"
require "json"
require "socket"
require "uri"
...
USER_ID = 1
TITLES_FILE = Rails.root.join("tmp", "favorite_titles.txt")
...
puts "Importing #{titles.length} title(s) for #{user.first_name} #{user.last_name} (id=#{user.id})..."
...
titles.each do |title|
  ...
  # Mutual exclusivity with Watchlist
  WatchlistMovie.where(user_id: user.id, movie_id: movie.id).destroy_all

  favorite = FavoriteMovie.find_or_initialize_by(user_id: user.id, movie_id: movie.id)
  if favorite.persisted?
    puts "already in Favorites (#{movie.title}, #{movie.release_year})"
  elsif favorite.save
    puts "ADDED (#{movie.title}, #{movie.release_year})"
  else
    puts "FAILED: #{favorite.errors.full_messages.join(", ")}"
  end
rescue => e
  puts "ERROR: #{e.class} - #{e.message}"
end

puts "Done. Favorites count: #{user.favorite_movies.count}"
